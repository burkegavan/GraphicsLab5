...
mat4 ortho(float left, float right, float bottom, float top, float near, float far) {
	...
	mat4 orth = identity_mat4();

	//diagonal
	orth.m[0] = (2.0f / (right - left));
	orth.m[5] = (2.0f / (top - bottom));
	orth.m[10] = ((-2.0f / (far - near)));
	orth.m[15] = 1.0f;

	//last column
	orth.m[3] = (((left + right) / (right - left)) * -1.0f);
	orth.m[7] = (((top + bottom) / (top - bottom)) * -1.0f);
	orth.m[11] = (((far + near) / (far - near)) * -1.0f);
